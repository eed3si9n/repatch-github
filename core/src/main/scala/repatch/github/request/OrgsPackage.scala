 ...
 */
final case class OrgsPackage(
    org: Orgs,
    packageType: String,
    packageName: String,
    params: Map[String, String] = Map()
) extends Method
    with Param[OrgsPackage] {
  override def complete: Req => Req =
    org.complete(_) / "packages" / packageType / packageName <<? params
  ...
  def deleteVersion(versionId: BigInt) = OrgsPackage.OrgsPackageVersionDelete(this, versionId)
...
object OrgsPackage {
  final case class OrgsPackageDelete(pkg: OrgsPackage) extends Method {
    override def complete: Req => Req = { req: Req =>
      pkg.org.complete(req.DELETE) / "packages" / pkg.packageType / pkg.packageName
    }
  }
  ...
  final case class OrgsPackageVersions(pkg: OrgsPackage, params: Map[String, String] = Map())
      extends Method
      with Param[OrgsPackageVersions]
      with PageParam[OrgsPackageVersions] {
    override def complete: Req => Req =
      pkg.org.complete(_) / "packages" / pkg.packageType / pkg.packageName / "versions" <<? params
